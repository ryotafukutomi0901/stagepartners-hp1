...
### style: `Proclaim`のキラーワード「主役は、その場所で挑戦する人たちだ。」を2行に分割

- **背景**: 1行だったキラーワードを「主役は、」と「その場所で挑戦する人たちだ。」の2行に改行し、両方を同じ強調サイズ・同じ余白にしてほしいという依頼。
- **対応**: `components/Proclaim.tsx`の`PROCLAIM_PARAGRAPHS`で該当行を`["主役は、", "その場所で挑戦する人たちだ。"]`の2要素に分割。既存のレンダリング構造(同一グループ内の行は追加マージンなしで連続表示)をそのまま利用しているため、余白は他の複数行グループ(1つ目の段落)と同じになる。強調スタイルの判定条件にも新しい2行分を追加。
- **確認**: `npm run build`成功。
